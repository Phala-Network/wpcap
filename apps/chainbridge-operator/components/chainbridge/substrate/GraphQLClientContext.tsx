...
export const SubstrateGraphQLProvider = ({
    ...
    const client = useMemo(() => (endpoint !== undefined ? new GraphQLClient(endpoint) : undefined), [endpoint])

    return <GraphQLContext.Provider value={{ client }}>{children}</GraphQLContext.Provider>
}

export const useSubstrateGraphQL = (): IGraphQLContext => useContext(GraphQLContext)
